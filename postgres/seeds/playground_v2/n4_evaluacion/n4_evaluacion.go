...
func Apply(tx *gorm.DB) error {
	// Ids del playground (constantes string) parseados una sola vez. El común
	// recibe uuid.UUID ya parseado; los ids son fijos → MustParseUUID es seguro.
	...
	studentAnaUserUUID := common.MustParseUUID(studentAnaUserID)
	studentBrunoUserUUID := common.MustParseUUID(studentBrunoUserID)
	studentCaroUserUUID := common.MustParseUUID(studentCaroUserID)

	adminMembUUID := common.MustParseUUID(adminMembID)
	teacherMembUUID := common.MustParseUUID(teacherMembID)
	studentAnaMembUUID := common.MustParseUUID(studentAnaMembID)
	studentBrunoMembUUID := common.MustParseUUID(studentBrunoMembID)
	studentCaroMembUUID := common.MustParseUUID(studentCaroMembID)

	offeringScienceUUID := common.MustParseUUID(offeringScienceID)

	// Escuela con GradeProfile "detailed" (N4 / ADR 0020): es lo que habilita el
	// desglose por componentes (grade_item) en "Mis notas" del alumno. A
	// diferencia de los demás playgrounds (default "basic"), aquí DEBE ser
	// "detailed" para ejercitar el cierre N4.
	...
	if err := common.SeedUser(tx, common.UserSpec{ID: adminUserUUID, Email: AdminEmail, Password: Password, FirstName: "Admin", LastName: "N4"}); err != nil {
		return fmt.Errorf("playground_v2/n4_evaluacion: admin_user: %w", err)
	}
	if err := common.SeedUser(tx, common.UserSpec{ID: teacherUserUUID, Email: TeacherEmail, Password: Password, FirstName: "Profe", LastName: "Ciencias"}); err != nil {
		return fmt.Errorf("playground_v2/n4_evaluacion: teacher_user: %w", err)
	}
	if err := common.SeedUser(tx, common.UserSpec{ID: studentAnaUserUUID, Email: StudentAnaEmail, Password: Password, FirstName: "Ana", LastName: "Alumna"}); err != nil {
		return fmt.Errorf("playground_v2/n4_evaluacion: student_ana_user: %w", err)
	}
	if err := common.SeedUser(tx, common.UserSpec{ID: studentBrunoUserUUID, Email: StudentBrunoEmail, Password: Password, FirstName: "Bruno", LastName: "Alumno"}); err != nil {
		return fmt.Errorf("playground_v2/n4_evaluacion: student_bruno_user: %w", err)
	}
	if err := common.SeedUser(tx, common.UserSpec{ID: studentCaroUserUUID, Email: StudentCaroEmail, Password: Password, FirstName: "Caro", LastName: "Alumna"}); err != nil {
	...
	if err := common.SeedUserRole(tx, studentAnaUserUUID, common.MustParseUUID(l4.L4_ROLE_STUDENT_ID)); err != nil {
		return fmt.Errorf("playground_v2/n4_evaluacion: student_ana_user_role: %w", err)
	}
	if err := common.SeedUserRole(tx, studentBrunoUserUUID, common.MustParseUUID(l4.L4_ROLE_STUDENT_ID)); err != nil {
		return fmt.Errorf("playground_v2/n4_evaluacion: student_bruno_user_role: %w", err)
	}
	if err := common.SeedUserRole(tx, studentCaroUserUUID, common.MustParseUUID(l4.L4_ROLE_STUDENT_ID)); err != nil {
	...
	if err := common.SeedMembership(tx, common.MembershipSpec{ID: studentAnaMembUUID, UserID: studentAnaUserUUID, SchoolID: schoolUUID, AcademicUnitID: &unitUUID, Role: "student"}); err != nil {
		return fmt.Errorf("playground_v2/n4_evaluacion: student_ana_membership: %w", err)
	}
	if err := common.SeedMembership(tx, common.MembershipSpec{ID: studentBrunoMembUUID, UserID: studentBrunoUserUUID, SchoolID: schoolUUID, AcademicUnitID: &unitUUID, Role: "student"}); err != nil {
		return fmt.Errorf("playground_v2/n4_evaluacion: student_bruno_membership: %w", err)
	}
	if err := common.SeedMembership(tx, common.MembershipSpec{ID: studentCaroMembUUID, UserID: studentCaroUserUUID, SchoolID: schoolUUID, AcademicUnitID: &unitUUID, Role: "student"}); err != nil {
	...
	if err := common.SeedOffering(tx, common.OfferingSpec{
		ID:                  offeringScienceUUID,
		SchoolID:            schoolUUID,
		SubjectID:           subjectScienceUUID,
		AcademicUnitID:      unitUUID,
		PeriodID:            periodUUID,
		SectionLabel:        &sectionA,
		TeacherMembershipID: &teacherMembUUID,
	}); err != nil {
	...
	if err := common.SeedEnrollment(tx, common.EnrollmentSpec{OfferingID: offeringScienceUUID, SubjectID: subjectScienceUUID, PeriodID: periodUUID, StudentMembershipID: studentAnaMembUUID}); err != nil {
		return fmt.Errorf("playground_v2/n4_evaluacion: enroll_science_ana: %w", err)
	}
	if err := common.SeedEnrollment(tx, common.EnrollmentSpec{OfferingID: offeringScienceUUID, SubjectID: subjectScienceUUID, PeriodID: periodUUID, StudentMembershipID: studentBrunoMembUUID}); err != nil {
		return fmt.Errorf("playground_v2/n4_evaluacion: enroll_science_bruno: %w", err)
	}
	if err := common.SeedEnrollment(tx, common.EnrollmentSpec{OfferingID: offeringScienceUUID, SubjectID: subjectScienceUUID, PeriodID: periodUUID, StudentMembershipID: studentCaroMembUUID}); err != nil {
