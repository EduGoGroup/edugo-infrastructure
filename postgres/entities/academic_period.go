...
type AcademicPeriod struct {
	...
	AcademicUnitID uuid.UUID `db:"academic_unit_id" gorm:"type:uuid;index;not null;constraint:academic_periods_unit_fkey,OnDelete:CASCADE" validate:"required,uuid"`
	...
	// NOTE: partial unique index idx_academic_periods_active ON (school_id, academic_unit_id) WHERE is_active = true must be created in post_gorm.sql
